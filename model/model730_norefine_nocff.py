...
class SimpleConvNet(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels=4, out_channels=16, kernel_size=1, padding=0),
            nn.BatchNorm2d(16),
            nn.ReLU()
        )
...
class Cross_Attn(nn.Module):
    def __init__(self, in_dim):
        ...
        self.query_conv1 = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=1)
        self.key_conv1 = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=1)
        self.gamma = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim=-1)  #

    def forward(self, x, y):
        m_batchsize, C, width, height = x.size()
        proj_query = self.query_conv1(x).view(m_batchsize, -1, width * height).permute(0, 2, 1)  # 1, 1024, 1
        proj_key = self.key_conv1(x).view(m_batchsize, -1, width * height)  # 1, 1, 1024

        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)  # BX (N) X (N)

        yproj_value = self.value_conv(y).view(m_batchsize, -1, width * height)  # 1, 512, 1024

        out = torch.bmm(yproj_value, attention.permute(0, 2, 1))
        out = out.view(m_batchsize, self.chanel_in, width, height)
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv=nn.Conv2d(513,512,1,1)
        self.crssatt = Cross_Attn(512)
        self.refine=SimpleConvNet()
        if encoder_freeze:
            self.freeze_encoder()
    ...
    def forward(self, *input):
        ...
        x, features, y, origin_input = self.forward_backbone(*input)  # 1,512,16
        y = F.adaptive_avg_pool2d(y, (x.size(2), x.size(3)))  # torch.Size([2, 1, 32, 32])
        x = self.crssatt(y, x)
        x=self.conv(torch.cat((x,y),1))
        # print('asjdhkjashdkja',x.shape)
        for skip_name, upsample_block in zip(self.shortcut_features[::-1], self.upsample_blocks):
            skip_features = features[skip_name]
            x = upsample_block(x, skip_features)

        x = self.final_conv(x)
        # out=self.refine(x,origin_input)
        
        return (torch.tanh(x) + 1.0) / 2.0

    def forward_backbone(self, x, y):
        ...
        return x, features, y,origin_input
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        with torch.set_grad_enabled(True):
            batch = torch.empty(1, 3, 512, 512).normal_()
            batch2 = torch.empty(1, 1, 512, 512).normal_()
            targets = torch.empty(1, 1, 512, 512).normal_()

            out = net(batch, batch2)
